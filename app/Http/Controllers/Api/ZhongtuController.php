...
class ZhongtuController extends CommonController{
    ...
    public function scan(){
        // $pageData = $this->httpRequest("http://www.bookschina.com/books/kinder/");
        $path =  base_path() . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'aotuba_img' . DIRECTORY_SEPARATOR . 'test' . DIRECTORY_SEPARATOR . 'test.txt';

        
        $data = \file_get_contents($path);

        $second_title_list = $this->getSecondTitle($data);

        $model = New \App\Models\ZhongtuBookTypeModel();

        $insert_data = [];

        foreach ($second_title_list as $key => $value) {
            $list = $this->getSecondTitle2($value);
            foreach ($list['title'] as $s_key => $s_value) {
                $index_data = $list['index'][$s_key];
                $index_array = explode('/',$index_data);
                array_pop($index_array);
                $index = array_pop($index_array);

                $is_repeat = $model->where('index',$index)->first();
                if(!$is_repeat){
                    $temp['index'] = $index;
                    $temp['pid'] = $key + 1;
                    $temp['name'] = $s_value;
        ...
        // $first_title_list = $this->getFirstTitle($data);

        // $model = New \App\Models\ZhongtuBookTypeModel();

        // $first_insert_data = [];
        // foreach ($first_title_list['title'] as $key => $value) {
        //     $index_data = $first_title_list['index'][$key];
        //     $index_array = explode('/',$index_data);
        //     array_pop($index_array);
        //     $index = array_pop($index_array);
        ...
        //         $first_insert_data[] = $temp;
        //     }
        // }

        // if($first_insert_data){
        //     $model->insert($first_insert_data);
        // }
        var_dump('123');exit;
        // echo $data;
        // exit;
